...
class BaseTests:
    ...
    def init_test(self, App: Application):
        from page_objects.abc_iview_tv.launch_screen import LaunchScreen
        from page_objects.abc_iview_tv.linktv_screen import LinkTvScreen
        from page_objects.abc_iview_tv.whos_watching_screen import WhosWatchingScreen
        from utilities.soft_assert import SoftAssert
        from utilities.utils import TVRemoteAction

        self.soft_assert = SoftAssert()
        self.tv_remote_action = TVRemoteAction()
        self.launch_screen = LaunchScreen(app=App)
        self.linktv_screen = LinkTvScreen(app=App)
        self.wws_screen = WhosWatchingScreen(app=App)
